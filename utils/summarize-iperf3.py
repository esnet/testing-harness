...
# For pscheduler, you must run the script "cleanup_pscheduler.py" first.
...
for root, dirs, files in os.walk(directory_path):
    if root != "." or dirs == []:
        ...
        print ("\n======= Starting in new directory: ", root)
        pacing_string = ""
        if not args.iperf3_json:  # ie: if pscheduler
...
#                   print ("Pacing set to: ", pacing_int)
                ...
                pacing_int = 0
            fq_rate = float(pacing_int) / 1000000000 
            ...
            if match:
                ...
                nstreams = 1
                print ("   parallel not found in src_cmd, setting nstreams to 1")
                print ("  ", src_cmd)
        ...
        if args.iperf2 or iperf2:
             if nstreams == 1:
                 ...
                 if sum_stream:
                     ...
                     print("   iperf2: Throughput (Gbps) for All Streams: %.2f" % gbits_per_second)
                 else:
                     print("   SUM Stream not found in the JSON data.",file_path)
                     gbits_per_second = 0

             retransmits = 0  # pscheduler does not capture retrans from iperf2

        else: # if iperf3
             # not sure why, but sometimes pscheduler includes a 'succeeded' value, and sometime not
             # but alway skip if succeeded = False
             if "succeeded" in json_data and json_data["succeeded"] == False:
                 print ("   Test failed, continuing to next file")
                 continue   # test failed do skip this file
             else:
                 try:
                     # older version of iperf3 is this (I think)
                     gbits_per_second = float(json_data["summary"]["summary"]["throughput-bits"]) / 1000000000
                     retransmits = json_data["summary"]["summary"]["retransmits"]
                 except:
                     try:   # newer iperf3 is different....
	                 ...
	                 # this should not happen...
                         print("\n Error: cant find json object [summary] or [end] ", file_path )
                         print (json_data["end"]["sum_sent"])
                         sys.exit()

             if VERBOSE:
                 print("   iperf3: Throughput (Gbps) for All Streams: %.2f" % gbits_per_second)
...
if json_file_cnt == 0:
    print ("No JSON data files found. Did you run cleanup_pscheduler.py? Or do you need the -i flag? ")
    sys.exit()
...
if args.include_mss:
    average_table_data = sorted(average_table_data, key=lambda x: (x[0], str(x[3]), x[2]))
    ...
    average_table_data = sorted(average_table_data, key=lambda x: (x[0], str(x[2]), x[4], x[3]))
